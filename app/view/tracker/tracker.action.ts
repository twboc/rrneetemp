...
export const getAllDomains = async (
  ...
  await getQueryStats(
    res.data.DomainPermissionWithDomain[0].domain,
    res.data.DomainPermissionWithDomain[0].domain_id,
    setStats,
  )
...
//@ts-ignore
export const chageSelectedDomain =
  (
    setSelectedDomain: Dispatch<SetStateAction<string>>,
    setStats: Dispatch<SetStateAction<ITrackerDomainStats>>,
    setQueryStatsLoading: Dispatch<SetStateAction<boolean>>,
...
const getQueryStats = async (
  ...
  const result = await resource.api.tracker.domain.get.stats({
    domain,
    domain_id,
  })
  ...
  payload.query = payload.query.map(el => {
    el.query_variant = el.query_variant.map(variant => {
      if (variant.query_variant_result) {
        variant.query_variant_result =
          variant.query_variant_result
            .filter(withPosition)
...
export const createDomain =
  //@ts-ignore
  (setInvalidDomain, selectedDomain, organisations, domains, setDomains) =>
    () =>
      addDomain(
        setInvalidDomain,
        selectedDomain,
...
export const addOnEnter =
  (
    queryInput: React.MutableRefObject<null>,
    domains: IDomainListed[],
    selectedDomain: string,
    locations: string[],
    organisations: IUserOrganisationByUser[],
    stats: ITrackerDomainStats,
...
export const getAddBulk =
  ...
  () => {
    const queries = bulkText.split(/\r?\n/).filter(el => el != '')
...
export const createQuery =
  ...
  () => {
    const domain = domains.filter(el => el.domain_id == selectedDomain)[0]
      .domain
    const queryData: IQueryCreate[] = queryConstructor(
      selectedDomain,
      query,
      locations,
    )
