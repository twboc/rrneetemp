...
type QueryCreateReq = Request<
  {},
  {},
  {domain: string; queries: IQueryCreate[]; organisation_id: string}
>
...
const queryCreate = async (req: QueryCreateReq, res: Response) => {
  ...
  const currentDomainOrder = await model.domain_order.getCurrentOrder({
    domain_id,
  })

  if (!currentDomainOrder.success)
    return console.log('!currentDomainOrder.success')
  ...
  const queryVariantResult =
    await model.query_variant.createMany(queryVariantInsert)
  ...
  const queryVariantOrderResult =
    await model.query_variant_order.createMany(query_variant_order)

  if (!queryVariantOrderResult.success)
    return console.log(
      '!queryVariantOrderResult.success: ',
      queryVariantOrderResult,
    )

  console.log(
    'currentDomainOrder: ',
    util.inspect(queryVariantOrderResult, false, null, true),
  )
...
export const domainCreate = async (
  req: Request<{}, {}, {domain: string; organisation_id: string}>,
  res: Response,
) => {
  ...
  const domainPermission = createDomainPermission(
    domain.id,
    token.data.token.id,
    req.body.organisation_id,
  )
  const domainPermissionResult =
    await model.domain_permission.create(domainPermission)
  if (!domainPermissionResult.success) {
...
export const getAllDomains = async (
  req: Request<{}, {}, {organisation_id: string}>,
  res: Response,
) => {
  const token = await auth.token(req)
  if (!token.success) return respond.user.init.fail.default(res)
  ...
  const result =
    await model.domain_permission.getDomainsByUserAndOrganisation(
      user_organisation,
    )

  if (!result.success) return

  const flat = result.data.DomainPermissionWithDomain.filter(
    el => !el.domain.tombstone,
...
const getAllQueryVariants = async (
  req: Request<{}, {}, {domain_id: string}>,
  res: Response,
) => {}

const getDomainAllStats = async (
  req: Request<{}, {}, {domain: string; domain_id: string}>,
  res: Response,
) => {
  const result = await model.domain.getStats({
    domain: req.body.domain,
